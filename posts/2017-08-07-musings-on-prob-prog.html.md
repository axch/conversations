---
title: Musings <em>on</em> Probprog
date: 2017/08/07
author: Alexey Radul
---

People in my circles periodically talk and write about the nature of
...
new scope of problems that can be solved by computing.  READMORE
...
communications satellite") may be generated.

To me, this shift is analogous to my college experience of shifting
...
$n > 0$ are there any triples of positive integers $x$, $y$, and $z$
such that $x^n + y^n = z^n$?  The answer is simple and clear:
$n \leq 2$.  The proof is an eighty-page book about elliptic curves that
not many people in the world can read, but the problem is considered
solved.

The standard of solution I learned from the computer science
...
In some sense, the computational standard is a relaxation of the
...
simply do not have simple, understandable answers.

The relationship between classical and probabilistic programming seems
...
do not have definite answers.

So what is the shape of this new scope of attackable problems?  What
...
to approach uncertainties in one's problem with computational objects.

As for solutions, and the internal workings of the discipline, I can
name several relatively novel features:

<dl>
<dt>Uncertainty in, uncertainty out.</dt>

<dd>Imprecisely posed questions
  necessarily have imprecise answers.  It _could_ be a US
...
<dt>Knowable unknowns.</dt>

<dd>Generally, the uncertainty within a given model
  ...
  (The [Central Limit Theorem](https://en.wikipedia.org/wiki/Central_limit_theorem) guarantees
  $O(\sqrt{N})$ accuracy for $O(N)$ computation for many questions,
  including probabilities.)  There are, of course, also many such
  probability questions to which the exact answer is known.</dd>

<dt>New problems, new tactics.</dt>

<dd>Expanding the space of approachable
  ...
  for the original question.</dd>

<dt>Inverse reasoning.</dt>

<dd>A specific problem template to which probability
...
<dd>It seems, looking at the
  ...
  processes as the in-model uncertainty does.</dd>

<dt>Robustness.</dt>

<dd>An interesting side-effect of models that explicitly
  ...
  and harder to diagnose errors in model specification.</dd>

<dt>Natural approximation.</dt>

<dd>To the extent that "a sample from a
  ...
  gap very much smaller than the model-reality gap.</dd>

<dt>Squishy Testing.</dt>

<dd>Testing a probabilistic program is [squishier](../../2016/on-testing-probabilistic-programs).
  ...
  down by computation.</dd>

What, then, is probabilistic programming?  It is the practice of

...
date: 2013/08/05
author: Alexey Radul
epigraph: The fox knows many things; the hedgehog knows one big thing.
---
The successful fox must know more than the sum of what the
...
solving certain kinds of problems.READMORE

I keep coming across domains with the following structure:

1. One wishes to solve a large, complex problem that fits into
   some uniform formalism (for example, a detailed simulation of a car
   as a system of 10,000 ODEs).

2. The formalism has a variety of methods developed for solving
   ...
   whole solution (e.g., the step size).

3. The problem, being large, has many different parts, with different
   solution characteristics (e.g., some variables change slowly and
   smoothly, others oscillate rapidly).

4. The performance of each hedgehog method on the whole problem is
   determined by its performance on the worst part of the problem
   (e.g., the step size has to be small enough to trace the
   highest-frequency oscillation in the model).

5. Different methods (and different settings of the global state) have
   different weaknesses (e.g., high-order integrators are good for very
   smooth systems, because they can achieve large step sizes).

It is therefore desirable to solve such large problems like a fox: partition
...
On the one hand, such foxy partitioning is eased because these
...
densely than with variables in a different clique.

On the other hand, partition is difficult, because the parts do
...
This pattern seems to appear quite often at the cutting edge of
computational science.  Some formalisms where this phenomenon is
encountered, with example problems:

- Systems of ordinary differential equations or differential algebraic
  equations (the car example, with parts of greatly differing
  stiffness).

- Coupled partial differential equations (e.g., detailed simulation of
  ...
  2D PDEs, and the tanks with 3D PDEs).

- Model inference in machine learning (different model shapes, like
  Gaussian mixtures and HMMs, can easily appear in the same compound
  model).

  - This is exacerbated by the rise of probabilistic programming
    languages, which easily generate models with long chains of
    deterministically-linked variables---the only place where
    exhaustive search shines as an inference method.

- Constraint satisfaction

One common theoretical thread is that all these problem domains are at
...
It is therefore desirable to develop foxy computer systems: ones that
...
How _to_ Breed _a_ Digital Fox
------------------------------

How might such a digital fox be developed?  It seems to me that a
system like this must have the following pieces:

- Problem language: A language for specifying instances of the problem
  class (e.g., the Modelica language for DAEs, various probabilistic
  programming languages).

  - This language must of necessity have means of abstraction, in
...
- Hedgehogs: A library of primitive solution methods (e.g., 4th order Runge-Kutta, Gibbs sampling).  I
  ...
  as to over-restrict applicability.

- Bridges: A library of bridges for communicating between solution methods.  In
  my mind, this is the key missing piece in all proposed foxes I have
  seen so far---more [below](#on-bridges).  In particular, I have seen no evidence
  ...
  knowledge exists even in the minds of practitioners.

- Plan language: A language for specifying problem-specific compound methods.  A
  ...
  simulation changing the applicable equations).

- Plan executor: A mechanism for efficiently executing problem-specific compound
  ...
  language of compound methods.

- Planner: A subsystem for evaluating, searching, suggesting, or
  inferring compound methods automatically based on the specification
  of the problem.

While it is important that these six pieces are different, and
...
together until a good solution emerges.

You will note that I omitted the planner from the previous admonition.
...
productivity to be tremendous.
 
In fact, I think the automatic method invention problem is something
...
By the way, I see making a digital fox as a form of defeat.  In some sense,
...
additional complexity for no "fundamental" gain in coverage.[^not-really]  This
...
The thing that makes some procedure a (hedgehog) solution method is
...
solution is bidirectional.

This problem has an easy version and a hard version.  The easy version
...
method into the information consumed by the other.

I can envision two kinds of bridges: ones that operate across a link
...
My understanding is that bridge-building is not currently well
...
variational method to infer another?

_Extant_ Foxes
--------------

When I started writing this essay, I didn't think I knew of any
...
The operating system kernel seems to stretch the analogy a little, but
...
don't know whether Linux is a fox in this sense.

For the mathematically inclined, a
[sheaf](https://en.wikipedia.org/wiki/Sheaf_%28mathematics%29) is the
only example I know about of a mathematical structure which is a fox.
The gluing axiom is analogous to the all-important bridges.

Notes
-----

[^not-really]: Actually, my main point, to the extent I have one, is that this is
